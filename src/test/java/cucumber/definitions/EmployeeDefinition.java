...
public class EmployeeDefinition {
    ...
    public void send_request_http(int statusCode) {
        assert response.statusCode() == statusCode : "Error, Actual status code not " + statusCode + " but " + response.statusCode();
    }
    ...
    public void assert_token_in_variable() {
        assert EmployeeDefinition.token != null : "Token null " + EmployeeDefinition.token;

    }

    @And("Full name in the response must be {string}")
    public void assert_full_name(String fullName) {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            List<AddEmployeeResponse> addEmployeeResponse = objectMapper.readValue(response.body().asString(),
                new TypeReference<List<AddEmployeeResponse>>() {});
            assert addEmployeeResponse.get(0).getFullName().equals(fullName) : "full name not expected";
        } catch (Exception e) {
    ...
    public void assert_department(String department) {
        ObjectMapper objectMapper = new ObjectMapper();
        try {
            List<AddEmployeeResponse> addEmployeeResponse = objectMapper.readValue(response.body().asString(),
                new TypeReference<List<AddEmployeeResponse>>() {});
            assert addEmployeeResponse.get(0).getDepartment().equals(department) : "department not expected";
        } catch (Exception e) {
            throw new RuntimeException("Failed to parse response body", e);
        }
    }
    ...
    public void assert_title(String title) {
        ...
        try {
            List<AddEmployeeResponse> addEmployeeResponse = objectMapper.readValue(response.body().asString(),
                new TypeReference<List<AddEmployeeResponse>>() {});
            assert addEmployeeResponse.get(0).getTitle().equals(title) : "title not expected";
        } catch (Exception e) {
            throw new RuntimeException("Failed to parse response body", e);
        }
    }
